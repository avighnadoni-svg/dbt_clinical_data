...
with src as (
    select * from {{ source('bronze', 'br_immunizations') }}
),
...
final as (
    select
        {{ dbt_utils.generate_surrogate_key([
            ...
            'src.CODE',
            'src.DATE'
        ]) }} as immunization_fact_key,
        p.patient_key,
        e.encounter_key,
        d.date_key as immunization_date_key,
        cc.clinical_code_key as immunization_code_key,
        src.DATE as immunization_ts,
        1 as immunization_event_count,
